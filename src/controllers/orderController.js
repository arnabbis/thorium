...
const createOrder= async function (req, res) {
    ...
    if(!user) return res.send('The request is not valid as no user is present with the given author id')

    if(!productIdd) return res.send("ProductId Must Be Present");

    let product = await productModel.findById(productIdd)
    if(!product) return res.send("The request is not valid as no product is present with the given author id")
